...
final class RouteAwareDictationRecorder: DictationAudioRecording {
    ...
    func stop() -> URL? {
        lifecycleQueue.sync {
            ...
            lock.unlock()
            // Both route slots stay prepared while idle; only active IO stops.
    ...
    private func selectRecorder(preferredInputDeviceID: AudioObjectID?) -> DictationAudioRecording {
        ...
        // The other route slot remains warm. A future selection can reuse its
        // graph, while explicit cancel/coolDown still invalidates both slots.
